...
function invariantBlock(c: ProposalCandidate): string[] {
  ...
  if (c.invariantTouches.length === 0) {
    ...
    lines.push(
      ...
      'operator to judge weaken-vs-amend — it is never auto-dropped.',
    );
    lines.push('');
  ...
  if (c.highRisk) {
    lines.push(
      ...
      '(today this is a manual judgment call; automating it is a planned future step).',
    );
    lines.push('');
  }
  return lines;
...
export function buildProposalBody(c: ProposalCandidate): string {
  ...
  lines.push('> **Auditor proposal.** Generated by `macf propose` ' +
...
export function buildReport(input: ReportInput): string {
  ...
  lines.push('');
  lines.push(
    '> The auditor PROPOSES only. Candidates below are corroborated ' +
    'across distinct agents; HELD candidates fell below the distinct-agent ' +
    'threshold. Nothing is auto-applied — the operator ratifies.',
  );
  lines.push('');
