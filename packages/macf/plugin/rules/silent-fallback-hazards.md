...
---
### Instance 21 — the test's scope excludes the seam where the defect lives: complete unit coverage, green CI, and the composition is broken (never-invoked AND never-sequenced)
...
**Two faces, one defect.** They share the root (test scope stops short of the seam) and the defense at the level this catalog states defenses — *assert at the composition boundary, not only at the unit* — which is why they share a number (the catalog's test: **a new number needs a distinct defense**). They are separated here because the wiring seam yields a silent no-op while the composition seam yields a loud failure in the wrong place, and a reader who meets only one will not think to check the other.
...
**Recurrence:** five sightings across one arc (`#862`, `#913`, `#920`, `#929` wiring; `#917` composition), cross-agent-confirmed. Notably it is the **most frequent** class in this catalog's most recent period, and every instance shipped through a green suite.
...
### Defense-pattern emergence (19-of-20 active instances have structural defense applied or shipped)
