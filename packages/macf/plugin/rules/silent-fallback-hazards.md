...
This rule names the CLASS so agents recognize the shape on first encounter rather than re-discovering each instance from scratch. Fifteen active instances are documented below as worked examples spanning different architectural layers (identity, parsing, TUI binding, observability routing, config substitution, multi-agent coordination protocol, metric-instrumentation lifecycle, observability-endpoint routing, release-pipeline-partial-publish, third-party-action retry-exhaustion, credential-refresh temporal-binding, multi-agent review-gate routing, notification-payload-anchor, channel-flag gating, CA-rotation out-of-band blast radius). (Instance 10 — a legacy substrate-routing receipt-gap — was retired 2026-06-07; its number is kept, not reused.) Fourteen of fifteen active instances have structural defenses applied or in flight — the pattern of defense generalizes alongside the pattern of hazard.
...
- **PREVENT — rotation-time blast-radius WARN.** `macf certs init`/`rotate` ends with a loud warning enumerating the orphaned out-of-band artifacts (routing-client cert on the caller-repo set, from the DR-030/DR-038 install-set) + the exact re-mint + re-set operator commands. Warn, never auto-write (DR-019).
...
### Defense-pattern emergence (14-of-15 active instances have structural defense applied or shipped)
...
Add as a new numbered section (the next number is **17** — numbering is append-only; retired instances keep their slot, see Instance 10) with the same fields: Surface / Failure shape / Recurrence / Defense status. Increment the intro paragraph's active-instance count + the Defense-pattern emergence header's `N-of-M active instances` count too.
