...
### Defense-pattern emergence (17-of-18 active instances have structural defense applied or shipped)
...
Add as a new numbered section (the next number is **20** — numbering is append-only; retired instances keep their slot, see Instance 10) with the same fields: Surface / Failure shape / Recurrence / Defense status. Increment the intro paragraph's active-instance count + the Defense-pattern emergence header's `N-of-M active instances` count too.
