...
**Recurrence:** 3 proxy checks + 1 forced-proxy on one surface (macf#872, verified by full audit), plus the same root in **Instance 16** (recorded-fingerprint proxy) and the **macf#855** attestation gap — well past a one-off; it is a *surface where the convention never landed*, not a set of isolated oversights.
...
**The in-tree template:** `macf fleet doctor` (DR-030) already has the correct posture — it live-probes mTLS and greens its ACCEPTED tier only on `HTTP 200 AND ack AND echoed correlation_token matches` ("the echo proves a real round-trip, not a coincidental 200"), then declares its own limit loudly (reaches-server ≠ delivered-to-agent) and ships `--inject` for the tier beyond. The fix for the static-GitHub-plane checks is to port that probe-first, honest-legend posture to them. Fixes in flight: macf#872 (check 4 first — the live-broken one).
...
### Defense-pattern emergence (15-of-16 active instances have structural defense applied or shipped)
...
Add as a new numbered section (the next number is **18** — numbering is append-only; retired instances keep their slot, see Instance 10) with the same fields: Surface / Failure shape / Recurrence / Defense status. Increment the intro paragraph's active-instance count + the Defense-pattern emergence header's `N-of-M active instances` count too.
