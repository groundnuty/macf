...
This rule names the CLASS so agents recognize the shape on first encounter rather than re-discovering each instance from scratch. Nineteen active instances are documented below as worked examples spanning different architectural layers (identity, parsing, TUI binding, observability routing, config substitution, multi-agent coordination protocol, metric-instrumentation lifecycle, observability-endpoint routing, release-pipeline-partial-publish, third-party-action retry-exhaustion, credential-refresh temporal-binding, multi-agent review-gate routing, notification-payload-anchor, channel-flag gating, CA-rotation out-of-band blast radius, health-check-proxy-not-invariant, routing-table dual-role, generator-build currency, shared-host subject misidentification). (Instance 10 — a legacy substrate-routing receipt-gap — was retired 2026-06-07; its number is kept, not reused.) Seventeen of eighteen active instances have structural defenses applied or in flight — the pattern of defense generalizes alongside the pattern of hazard.
...
- **Modal auto-clear (macf#703 / DR-033 auto-responder) — covers sub-shape B.** The prompt-auto-responder should clear `/login` + the rating survey (+ ceremony prompts) so the input line is ready when a wake lands; its seed set must cover these, not just the channels prompt.
...
### Instance 19 — a stale-build generator emits a degraded artifact and reports success, carrying the authority of "generated"

**Surface:** any artifact-generating CLI command run from a stale `dist/` — `macf repo-init` concretely, and by extension every generator whose output is a durable, security-relevant file.
...
**Recurrence:** confirmed `macf#886` (2026-08-12); the second manifestation of the `macf#144` stale-dist hazard class, whose first instance motivated the build-info staleness warning on `macf update`.
...
**Pattern:** A (emit-time result-invariant assert) + staleness-detection promoted to the generator boundary.

---


### Instance 20 — a guard locates its own subject by scanning a shared host, and measures a PEER: right measurement, wrong subject

**Surface:** any check that must find "my own" artifact (log, socket, pidfile, port, state dir) on a host shared with peer agents — here the SessionStart guards `check-channel-alive.sh` / `check-channels-enabled.sh` resolving their channel-server log.
...
### Defense-pattern emergence (18-of-19 active instances have structural defense applied or shipped)
...
| 3 — tmux-wake keystroke swallowed by an alternate input consumer (RC-IPC-binding **or** a modal input state: `/login`, rating survey, ceremony prompt — verified 2026-07-05) | Claude Code TUI input | Load-bearing = native channels (macf#641 — delivery independent of tmux input state, covers both sub-shapes); modal auto-clear (macf#703/DR-033) covers the modal sub-shape; consumer-fleet content-drop retired via channel-server HTTPS POST (DR-020); substrate = rule-discipline + Pattern C fragility detector | Pattern B (native channels — bad delivery-path unrepresentable) + Pattern C (fragility detector) |
...
Add as a new numbered section (the next number is **21** — numbering is append-only; retired instances keep their slot, see Instance 10) with the same fields: Surface / Failure shape / Recurrence / Defense status. Increment the intro paragraph's active-instance count + the Defense-pattern emergence header's `N-of-M active instances` count too.
