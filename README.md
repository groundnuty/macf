...
> **Routing-stage note.** Stage 3 (`macf-actions@v3`, mTLS) is the canonical transport for **new consumer projects**, and the **CV fleet** is migrating onto it. The **substrate** (`groundnuty/macf`, `macf-science-agent`, `macf-devops-toolkit`) stays on the **v1.x SSH/tmux primitive** (`macf-actions@v1.3.x`, currently `v1.3.5`) — permanent for now and actively maintained, per the substrate-permanent-Stage-2 directive (`macf#273`); it is **not** being retired. (`#467` retired tmux-send-keys routing from the canonical **product/rules default** — i.e. new projects are Stage-3 — *not* from the substrate's operational transport.) The two stages don't interoperate, so an agent joining a fleet should match that fleet's **current** `agent-router.yml` stage — check it before pinning rather than assuming `@v3`. See [`design/macf-consumer-onboarding.md`](design/macf-consumer-onboarding.md) §"Routing-stage".
...
Releases are tag-versioned per repo; consumers pin to major tags (`@v3` for Stage-3 routing — new projects + the migrating CV fleet — currently at `v3.3.0`; the **substrate** stays on the v1.x SSH/tmux primitive `@v1.3.x`, currently `v1.3.5`, per the substrate-permanent-Stage-2 directive) and to exact versions for the plugin (currently `0.2.35`). Design rationale in [DR-013](design/decisions/DR-013-plugin-versioning.md).
