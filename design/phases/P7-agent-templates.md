...
12. **Escalation chain: code-agent → science-agent → user.** When code-agent is stuck after attempting to resolve, @mention science-agent (the coordinator), NOT the user. Science-agent triages and escalates to user only when needed. Reason: preserves the peer-coordination dynamic and insulates the user from low-level coordination noise.
