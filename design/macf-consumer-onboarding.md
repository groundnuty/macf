...
- [ ] **`groundnuty/macf-actions` workflow is on v3.x** (already shipped — `agent-router.yml` v3 is "registry-driven mTLS transport"; SSH+tmux paths gone from active code per `macf#257` P6 finding). The consumer's project repo must reference it from `.github/workflows/agent-router.yml` via `uses: groundnuty/macf-actions/.github/workflows/agent-router.yml@v3` (or pinned tag).
  ...
  > **⚠ Routing-stage note (transitional — read before pinning).** Stage 3 (`@v3`, mTLS) is the canonical transport for a new consumer project. The substrate / CV fleet (`groundnuty/macf`, `macf-science-agent`, `macf-devops-toolkit`, …) currently runs a legacy **v1.x SSH/tmux routing primitive** (`macf-actions@v1.3.x`, latest `v1.3.4`) but is **migrating to Stage 3** (the v1.x path is being retired, #467). The two stages do **not** interoperate, so **while the migration is in progress**, an agent that must coordinate with the fleet should match the fleet's **current** `agent-router.yml` stage — check it before pinning, rather than assuming `@v3`. The end-state is `@v3` fleet-wide; once an agent's fleet is fully on Stage 3, pin `@v3`. (`macf repo-init` scaffolds `@v3` by default.)
