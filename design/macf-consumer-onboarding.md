...
- [ ] **`groundnuty/macf-actions` workflow is on v3.x** (already shipped — `agent-router.yml` v3 is "registry-driven mTLS transport"; SSH+tmux paths gone from active code per `macf#257` P6 finding). The consumer's project repo must reference it from `.github/workflows/agent-router.yml` via `uses: groundnuty/macf-actions/.github/workflows/agent-router.yml@v3` (or pinned tag).
  ...
  > **⚠ Routing-stage note (read before pinning).** Stage 3 (`@v3`, mTLS) is the canonical transport for a new consumer project, and the **CV fleet** is migrating onto it. The **substrate** (`groundnuty/macf`, `macf-science-agent`, `macf-devops-toolkit`) stays on the **v1.x SSH/tmux primitive** (`macf-actions@v1.3.x`, latest `v1.3.5`) — permanent for now and actively maintained, per the substrate-permanent-Stage-2 directive (`macf#273`); it is **not** being retired. (`#467` retired tmux-send-keys from the canonical product/rules default, *not* from the substrate's operational transport.) The two stages do **not** interoperate, so an agent that must coordinate with a fleet should match that fleet's **current** `agent-router.yml` stage — check it before pinning, rather than assuming `@v3`. (`macf repo-init` scaffolds `@v3` by default for new projects.)
- [ ] **`@groundnuty/macf` CLI** is installable globally — `npm install -g @groundnuty/macf` resolves to current published version (**v0.2.35** as of 2026-06-05). Latest reachable via `npm view @groundnuty/macf version`. The doctor + Path-2 hooks set shipped in v0.2.9; later versions inherit + extend (A2A surface, version-aware collision takeover, close-keyword + turn-receipt routing hooks, channel-server version-pinning at `init` — see `CHANGELOG.md`).
...
- [ ] **The `<PROJECT_SEG>_CA_CERT` variable is set as a REPO variable on each agent (caller) repo** (e.g. `academic-resume` → `ACADEMIC_RESUME_CA_CERT`) — the v3 router reads the CA it trusts for the mTLS POST from `vars[<PROJECT_SEG>_CA_CERT]` on the **repo running the workflow**, NOT from the registry. `macf certs init` uploads the CA cert to the *registry* (backup + discovery); you must **also** set it as a repo variable on each agent repo — it is a public cert, so a variable, not a secret: `gh variable set <PROJECT_SEG>_CA_CERT --repo <owner>/<repo> < ca-cert.pem`. Skipping it fails `route-by-label` with `CA-cert var empty` (hard) or silently no-delivers — the confirmed root of a consumer-fleet routing outage (macf#806; the durable fix is macf-actions#66, which moves the CA read to the registry so no per-repo copy is needed). `<PROJECT_SEG>` = project name uppercased, hyphens→underscores.
