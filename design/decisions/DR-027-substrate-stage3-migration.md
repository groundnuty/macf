...
5. **The registry moves from the `.github/agent-config.json` file to GitHub Variables** (DR-005/006/007), **Profile scope** — the special `groundnuty/groundnuty` profile repo's (hidden) variables serve as the user-account registry: `registry-api-path = /repos/groundnuty/groundnuty`. **One registry for the fleet**; each agent's channel-server `host:port` is a variable there. *(Corrections 2026-06-25: `groundnuty` is a **user account** [`/orgs/groundnuty` → 404], so NOT org scope. An interim erratum [macf#528] said per-caller-repo — re-reading **DR-006** corrects that to its **Profile mode**: GitHub has no user-level variables, but the `user/user` repo accepts hidden repo variables acting as a user-level registry [DR-006, empirically validated; repo confirmed to exist]. This **restores the single-registry benefit** #528 thought was lost. Requires the `MACF_ROUTING` App **installed on `groundnuty/groundnuty`** to read it.)* (The old file may remain but is unread under v3.)

6. **Per-agent Stage-3 infra (devops-owned; resolved with devops on #524):**
   - **Topology (corrected per devops's channel-server grounding 2026-06-25):** the channel-server is **NOT a standalone daemon — it is an MCP stdio child of each agent's Claude Code** (spawned by the `macf-agent` plugin's `mcpServers` npx entry; **DR-002** makes MCP-stdio 1:1 with one Claude Code process). So the **supervision target is `claude.sh`/tmux** (already running), not a per-server systemd unit: *"stand up agent X's channel" = "launch X's `claude.sh`,"* and the server self-registers as its MCP child. Still one server per agent, **distinct ports** (DR-007 dynamic assignment), **one shared CA**; all five MACF-project homes co-tenant on `orzech-dev-agents` (auditor = 5th tenant → canary-zero validates the co-tenant layout directly).
   ...
   - **Secret placement + rotation:** `ROUTING_CLIENT_CERT/KEY` + `MACF_ROUTING_APP_KEY` are **per-repo secrets** on each of the 4 caller repos (*secrets* can't use the profile-repo trick that the *registry Variables* do — so secrets stay per-repo; `MACF_ROUTING_APP_ID`/`_KEY` already set per-repo 2026-06-25). The `MACF_ROUTING` App must also be **installed on `groundnuty/groundnuty`** so it can read the Profile-scope registry. Secret rotation re-pushes to the 4 caller repos (a `for` loop). **Asymmetric cadence:** server leaf certs auto-rotate on the VM under the long-lived CA (no GH touch); the client cert rotates on the DR-004 default; the CA rotates only on compromise.
   - v3 caller inputs: `project` (required) + `registry-api-path` = **`/repos/groundnuty/groundnuty`** (the Profile-mode registry — NOT the `/orgs/...` default, NOT per-caller).
   - **`macf-devops-toolkit` DR-005** (`DR-005-stage3-channel-infra-substrate-canary`, devops's *own* series — **not** macf-canonical; qualify cross-repo refs as "devops-toolkit DR-005") is the **single source of truth for the infra *mechanics***: the `claude.sh`/tmux launch (+ optional systemd-wrap of `claude.sh`), the MCP-child server lifecycle, CA/cert paths, the port map, and registry population. **DR-027 here states the migration *decision* + *scope* (Profile registry, per-repo secrets, canary order); it deliberately does NOT restate the mechanics** — devops-toolkit DR-005 consumes these decisions. (This division stops infra detail drifting in two docs — the cause of this DR's repeated §6 errata.)
...
**IN:** the routing transport of the 4 MACF-project agent homes (auditor + science/code/devops), canary-sequenced; registry-as-Profile-Variables (the `groundnuty/groundnuty` repo); the channel-server / mTLS-cert / `MACF_ROUTING` infra.
