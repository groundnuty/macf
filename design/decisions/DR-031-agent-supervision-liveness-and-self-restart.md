...
**The fix (shipped — `macf-devops-toolkit#128`): an execute-time aliveness-gate before Tier-2.** Sample the agent's tmux pane over a short window; **if it shows activity (agent busy) → ABORT the restart** (never apply a destructive recovery to a working agent); no-session → genuinely gone → restart proceeds. Held behind `--allow-restart` AND the gate (defense-in-depth). Fail-safe: a false-"busy" merely delays a needed restart (recoverable next sweep); a dead agent has no session → not busy → restart proceeds. **⚠️ The busy-signal MUST be `capture-pane -p | md5sum` content-diff, NOT `#{session_activity}` — see the verify-primitive correction below; the original `#128` shipped on `session_activity`, which does not detect an output-busy agent and so would have SIGTERM'd a working one.**
...
## Amendment (2026-06-28, `macf#645`) — verify-primitive correction: the busy/aliveness signal is capture-pane content-diff, NOT `#{session_activity}`

The aliveness-gate above (and the Tier-1 ladder gate) originally keyed on `tmux #{session_activity}`. **That primitive is wrong for liveness/busy detection — verified empirically.** A controlled test (detached session, 5 s of pure streaming output, no input) found `#{session_activity}` **stayed stable** — it does **not** advance on an output-busy agent (spinner / streaming / tool-renders), and in that test did not advance on a `send-keys` either (the precise trigger is murky, likely a detached-session quirk). The firm, verified conclusion: **`#{session_activity}` cannot distinguish "working" from "dead" for an agent producing output.**

**Consequence (why this is a correction, not a refinement):** the `#128` aliveness-gate as shipped — `session_activity` advanced → "busy" → ABORT restart — would read a **working, output-busy agent (the common case)** as **"not busy" → Tier-2 SIGTERMs it.** That is the exact near-miss the gate was built to prevent; on the old primitive the gate did not actually prevent it.

**The correct primitive: `tmux capture-pane -t <sess> -p | md5sum` over a short window** — a changed hash means real pane activity (output OR echoed input), which reliably detects a busy agent (verified: it caught the output the stable `session_activity` missed). This applies to **both** DR-031 gates (Tier-1 inject-confirm + Tier-2 aliveness) and is the canonical Pattern-C correction (`silent-fallback-hazards.md`, same date). The `#128` implementation is being fixed to capture-pane-diff in lockstep (`macf-devops-toolkit#128` follow-up).

**Meta:** `#{session_activity}` had been reached for as the "did it land / is it busy" primitive in several places (Pattern C, the Tier-1/Tier-2 gates, and the inject-delivery confirms in `macf#121`/#585 — the last two are a distinct "did a keystroke register" question, flagged for separate verification). One verify-primitive, wrong, propagated across surfaces — caught only by an empirical pane test. Verify the primitive, not just the logic built on it.
