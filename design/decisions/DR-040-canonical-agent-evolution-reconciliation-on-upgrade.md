...
1. **Tier first (cheap, avoids the expensive path).** For each dirty file, compare its content to what `macf update` *would* write (canonical for this workspace). **Already-canonical** (the common case — a stale branch behind the release, e.g. code-agent *and* science on v0.2.48's roll) ⇒ auto-resolve (commit/no-op), no agent involvement. Only a **genuine local delta** proceeds to the agent reconcile. *(Empirically validated: both code-agent and science hit only the already-canonical tier on the v0.2.48 roll — the front-loaded tier is the dominant case.)*
   - **Implementation prerequisite — a canonical-COMPUTE primitive (science review):** "what `macf update` would write" must be computed **without mutating the workspace** (a dry-run/compute mode). For **static** files (rules/settings) that is fetch-canonical + apply the managed-header. For **generated** files (`claude.sh` from `claude-sh.ts`) there is no static canonical to diff against — the tier-check must **generate-to-temp + diff** (or inspect whether the *generator* emits the feature), not fetch-and-diff. Generated files are the hard case; the compute primitive must handle both.
...
4. **Restart into a FRESH session** (Decision 5) with a **reconcile prompt injected in place of the normal startup issue-queue** — the prompt: *"You were upgraded to <version>. Your local edits to <files> were stashed as `<stash-name>`. For each edit: consult your **memory + reflection ledger** for its rationale, then re-apply the ones that are genuine evolution on top of the new canonical, discarding the rest. Re-applying may **conflict** where your edit and the refreshed canonical touch the same region — that is the keep-evolution-vs-take-canonical judgment at the hunk level; resolve it, or HALT if unsure. If you are UNSURE whether a change should survive, HALT and leave the stash for the operator — do not guess. When done and the tree is clean, drop the stash (`git stash drop <ref-matching <stash-name>>`)."*
   - **Why fresh-session is correct — the principled reason (science review), stronger than "self-contained":** genuine self-evolution is, by the framework's own disciplines (`codify-at-decision-time`, `codify-at-correction-time`, `reflection-staging`), **durable** — a real rule-tightening or hazard-codification leaves a trail in **memory + the reflection ledger + git history**, all of which a *fresh* session loads at startup. So the rationale a summarized-slice would carry is already persisted where the fresh session finds it. Conversely, an edit whose rationale exists **only** in the ephemeral conversation is by definition *uncodified* → not-yet-genuine-evolution → correctly discard-or-HALT. Fresh-session isn't just cheaper; it's architecturally aligned with how evolution is supposed to persist. Hence the explicit "consult memory + reflection ledger" step above, and the explicit hunk-level conflict-handling.
...
- **Deeper:** `-c` is inherently fragile — *any* interleaved session (reconcile, a debug launch, a raced watchdog) breaks "latest = mine." The robust long-term direction is agents resuming by a **persisted operational-session-id**, not `-c`. **Scope (science review): adopt it in the reconcile flow FIRST, then graduate framework-wide** — the reconcile *creates* the interleaved session, so it's the natural first adopter and de-risks the broader move (prove-then-distribute, the substrate-evolution cadence). The maintenance-lock (Decision 4) protects the window meanwhile.
...
The reconcile (Decision 3 step 4) is the agent deciding what of its local evolution to carry onto canonical — the agent acting as **its own auditor for its own workspace**. The mods it keeps are the **graduation signal**.

**Reuse the reflection ledger — don't invent a parallel surface (science review, `check-before-propose §4`):** a kept-edit **IS** a `rule_evolution_signal` — the existing field in the `reflection-staging.md` schema. So the reconcile emits each kept-edit as a `rule_evolution_signal` record into the **existing** ledger, where it composes *for free* with the fleet auditor's aggregation (DR-026) on fleets that have one. For consumer fleets with **no** local auditor (the auditor exists only in the substrate fleet today), a **`macf graduation-report`** command reads the ledger's `rule_evolution_signals` and files them as a graduation-candidate issue on `groundnuty/macf`. So agent-driven evolution flows up **without** every fleet needing its own auditor, and **without** a second evolution-signal surface. This is the concrete answer to "should consumer agents drive framework evolution, and how": yes — locally via reconcile → `rule_evolution_signal` in the ledger → fleet-wide via the auditor (or `macf graduation-report`).
...
## Open questions — RESOLVED during review (science + devops, PR #748)

- **✅ Maintenance-lock home + shape (devops):** portable VM (lock file `.claude/.macf/maintenance.lock`) ↔ K8s (pod annotation / ConfigMap); heartbeat-refresh + short TTL (not one fixed TTL); durable write before the stop. devops owns the standalone VM/cron build. → Decision 4 pins.
- **✅ Fresh-session vs summarized-slice (science):** FRESH — for a principled reason: genuine evolution is durable (memory + reflection ledger + git), which a fresh session loads; conversation-only rationale is uncodified → correctly discarded. Reconcile prompt now consults memory + ledger explicitly. → Decision 3 step 4.
- **✅ Graduation-candidate report (science):** reuse the reflection ledger — a kept-edit IS a `rule_evolution_signal`; `macf graduation-report` files them for auditor-less fleets. No parallel surface. → Decision 7.
- **✅ `-c` → session-id scope (science):** reconcile flow first, then graduate framework-wide (prove-then-distribute). → Decision 5.
