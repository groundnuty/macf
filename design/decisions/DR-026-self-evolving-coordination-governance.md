...
- **Who:** a **dedicated meta-auditor home, `groundnuty/macf-auditor-agent`** (one repo per agent) — *superseding* the original "science-agent wears the hat" lean (see **Amendment 2026-06-25** below). It governs MACF's evolution from its own contention-free home, holding the design rationale needed to review a universal-rule change well. It is **review, not self-review** (upstream proposals are externally authored).
- **Guardrail:** the auditor reviews *externally-authored* upstream PRs (fine), but must **not** be the sole reviewer of MACF's *own internally-authored* rule changes (self-review trap → route those to code/devops/operator). The operator stays the constitutional ratifier for all universal changes regardless. *(A dedicated `macf-auditor-agent` identity, distinct from science/code/devops, strengthens this — the auditor is now a separate actor from the implementers it observes, not science-agent double-hatting.)*
...
- **~~YAGNI + recursion requirement~~ (superseded — see Amendment 2026-06-25):** *originally:* start with `macf-science-agent` wearing the hat; split a dedicated meta-auditor home only if load or status-quo bias demands. MACF-the-project must eventually run its own project-auditor (the dogfooding that completes the picture).

> **Amendment 2026-06-25 (operator-ratified) — dedicated meta-auditor home now, not science-wears-the-hat.**
> The operator's 2026-06-24 call established **one repo per agent**, with the meta-auditor getting a **dedicated, project-scoped home repository** (`groundnuty/macf-auditor-agent`, created) rather than `macf-science-agent` double-hatting. This **exercises §8's original "split a dedicated home if demanded" escape immediately** — the original YAGNI lean is superseded, not contradicted. **Rationale:** (1) the home+identity atom — an agent is a persistent home + a *distinct external identity*; a dedicated App/home gives the meta-auditor a clean, contention-free workspace and an attribution-distinct actor; (2) it sharpens the self-review guardrail above — a separate `macf-auditor-agent` identity proposing, with science/code/devops reviewing, is cleaner separation than science-agent proposing-and-governing; (3) consistency with the fresh-deployment topology, where the auditor was always the 4th dedicated home. The shipped auditor identity/operating template (`plugin/agents/auditor.md`, macf#522) encodes this model; this amendment makes the ratified DR match it.
