...
### Amendment P — channel-server mounts via project `.mcp.json` (`server:`), not the plugin's `mcpServers` (added 2026-06-28, spike-proven `macf#641`; stdio-coupling + Path-B `macf#642`/#643)

**The channel-server moves from the plugin manifest's `mcpServers` to a project `<ws>/.mcp.json` MCP server.** Spike-proven on auditor (2026-06-28): native Claude Code channel-push **never worked for any macf agent** because a `--plugin-dir`-mounted plugin's channel id is `plugin:<name>:<server>` (e.g. `plugin:macf-agent:macf-agent`), and the `--dangerously-load-development-channels` dev-flag **rejects** that colon-triple form; the `plugin:<name>@<marketplace>` form is allowlist-enforced (incompatible with project-isolation); **only `server:<name>` — a manual `.mcp.json` MCP server — is dev-flag-loadable for a non-allowlisted channel** and stays project-scoped. (This is why every agent's only working delivery all along was the tmux-wake fallback, masking the dead native path — see silent-fallback-hazards Instance 15.)

- **Mechanism:** `macf init/update` writes `<ws>/.mcp.json` with `macf-agent` = `npx -y @groundnuty/macf-channel-server@<pinned>` **plus the OTEL env block** (env does NOT inherit across the MCP spawn boundary — `macf#422` — so the telemetry config must live in the `.mcp.json` server's `env`, not be assumed-inherited). Launcher keeps `--dangerously-load-development-channels server:macf-agent` (now resolves). Settings `enabledMcpjsonServers: ["macf-agent"]` pre-approves so launch doesn't stall on the server-approval prompt.
  **Asserted by:** `packages/macf/test/cli/mcp-json.test.ts` → `"the launcher's server:<name> argument equals a key macf init writes to .mcp.json"`
- **Mechanism (plugin-manifest strip) — NOT YET IMPLEMENTED at the source:** `mcpServers` is **stripped from the plugin manifest**; the plugin keeps skills/agents/hooks/rules via `--plugin-dir`. `macf#995` strips it from the LOCALLY-FETCHED copy on every `init`/`update`, which is what stops a second channel-server spawning — but the canonical manifest lives in `groundnuty/macf-marketplace`, and until a PR there drops `mcpServers` (**tracked: `macf#1003`**), this clause is only half-done. **Deliberately carries no `Asserted by:` line — its absence, in contrast to the clause above, IS the signal (`macf#998` Ruling 2).**
- **The version pin moves with it (supersedes the plugin-manifest pin for the channel-server):** the `@<pinned>` in the `.mcp.json` `npx` args is now the channel-server's pin (DR-013's plugin-versioning no longer governs the channel-server; it still governs the `--plugin-dir` plugin's skills/agents/hooks/rules). Lock-step versioning (Amendment D) still holds across the three npm packages.
- **Tool namespace flips** (see DR-023): `mcp__plugin_macf-agent_macf-agent__*` → `mcp__macf-agent__*`; Stop-hook `server:"plugin:macf-agent:macf-agent"` → `"macf-agent"`; pre-approved permission patterns updated in lockstep.

**Why ONE relation test, not two assertions:** the flag half (`claude.sh`) and the mount half (`.mcp.json`) are checked AGAINST EACH OTHER, because checking them separately is exactly how this amendment's mechanism went half-implemented for two months (the flag shipped `macf#632`; the mount never did, until `macf#995`). `claude.sh`'s `channelNotificationsLines()` and `.mcp.json`'s writer both derive the server name from the SAME exported constant (`mcp-json.ts::MCP_SERVER_NAME`), so the two can no longer drift apart by construction — the test is the backstop, not the only defense.

**Stdio-coupling fragility — chosen path is harden-stdio; Path B (HTTP/SSE) considered + dropped on complexity (`macf#642`/#643).** The `.mcp.json` server is a **stdio MCP child of Claude Code** — its survival is coupled to the main agent's activity: under heavy main-thread load CC stops servicing the stdio pipe, deems the connection dead, and kills the child, which is **never auto-respawned** (observed: code-agent's server died silently at 7.4 h under ~880k-token load; science's idle server ran 9.5 h). The `.mcp.json` mount (above) does not address this — it's orthogonal (it fixes channel *registration*; this is *lifecycle*).

**The chosen direction (operator, 2026-06-28) is to HARDEN the stdio child, NOT to re-architect the transport.** Verbatim: *"I do not like B anymore… 'Standalone service; CC connects to it' is getting too complicated. I prefer 'CC spawns + owns it as a child'… first let's harden it and not make it more complicated."* So:
- **`macf#642`/#643 (guaranteed file-based forensic log + top-level crash handlers + async-responsiveness hardening) is the path** — keep CC-spawns-and-owns-the-child (plug-and-play distribution, no separate service to run), and make the child robust + diagnosable. The diagnostics land **first** so the next death records its own cause.
- **Path B (HTTP/SSE — a standalone service CC connects to) was considered + DROPPED** for now on complexity. It remains the **leading revisit IF** the `#642` reproduction shows hardening is insufficient.
- **Whether hardening is a FULL fix or only mitigation is an OPEN question the `#642` reproduction decides — not yet settled** (verify-before-claim): if the death is the **child blocking its own event loop** (sync ops), async/responsiveness hardening is a **full fix**; if CC **starves the pipe regardless** of child responsiveness, hardening is mitigation and Path B becomes the leading revisit. (Science's standing hypothesis — stdio-coupling may be fundamental — is legitimate but unproven; the reproduction is how we find out.)
- **DR-031 interaction:** the supervision watchdog's mTLS `/health` probe *detects* a stdio death and relaunches (HEAL) regardless of root cause — that's a correct backstop either way; whether it's the *only* needed fix (harden suffices) or a stopgap (Path B needed) is the same open question. The B-vs-harden final call is the operator's; this DR ratifies with their nod.

**Rollout (per `macf#641`):** spike ✓ → full `.mcp.json` config on ONE agent (auditor, already spiked) + **live-verify in a real TTY** (NOT `claude -p` — it can't see channel activation; the dev-flag + resume-summary prompts are interactive, `anthropics/claude-code#42486`) → fleet via `macf init/update` + relaunch. **Per-agent live-verify is mandatory** — the entire notification-death investigation is the monument to why offline/headless verification misses this.

Status note: this amendment records the spike-proven + operator-directed (`#641` "commit to native channels") design; the migration ratifies with DR-013/022/023 before the fleet rollout.
