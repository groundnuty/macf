...
- **Passive-Processed (the better-than-inject default):** the agent self-reports *its own* last-processed-real-receipt from **local** state (a `/health` field), sourced from DR-025 comms-ledger / #444 / #45 / the turn-receipt hook. A receipt for a real routed message proves the full chain at **zero cost**. **Phase-1 precondition — locality *and* granularity (devops):** passive stays topology-agnostic in the mesh layer **only if** each agent can self-report a per-agent/per-message *local* processed-receipt. If processed-receipts live **only centrally** (Tempo/#444), passive can't be a self-report → it degrades to the routing-infra layer (deployment-specific) and `--inject` carries more mesh weight. The local self-report is the cleaner design — keep passive in mesh, gated on this.
  ...
  **Amendment (2026-06-26, operator-approved; precondition RESOLVED NEGATIVE → local-receipt path).** The precondition was verified and **fails**: no per-agent *local* processed signal exists today — `emit-turn-receipt.sh` only POSTs an OTel span (no local write), the comms-ledger writes `processed: null` at `/notify`-time ("unknown-at-write", `comms-ledger.ts:185`), and "processed" is reconstructed only by the #444 reconciler joining router-log ⋈ Tempo-spans (a *backend* query). So **both** passive (mesh self-report) **and** `--inject` (local processing verification) lacked their required local signal. Rather than degrade passive to routing-infra (the branch above), the operator approved the cleaner fix: **add a local-receipt path** — `emit-turn-receipt.sh` *also* appends a minimal `{ts, run_id, agent}` line to a local `processed-receipts.jsonl` sink that the channel server reads for a new `/health.last_processed` field. This makes passive a real **local** self-report (so it stays in the mesh layer as designed), lets `--inject` verify processing locally (`correlation_token = run_id`, the contract `--inject` matches against), and is **DR-025-aligned** (it completes DR-025's local-authoritative posture for the *processed* dimension — currently the one dimension that is null-locally + central-only). The receipt sink is a **separate file** from `comms-ledger.jsonl` (decoupled so the bash hook can't corrupt the rich authoritative ledger schema), and the local span is **retained** (still feeds Tempo + the #444 reconciler) — the receipt is *additive*. Decoupling the #444 reconciler from Tempo is a **future option the sink enables, NOT part of 1e**. Built as phase-1 increment **1e** (E1 branch). This closes the open question §3/§5 flagged.
...
1. **Phase 1 — mesh layer:** `/health` self-report extension (§5, `otel` from live process env, server-scoped) + `macf fleet status` + the mesh delivery checks (Reachable/Accepted/Passive-Processed, `--inject` fallback) + idle/busy. *(The passive-proof precondition RESOLVED NEGATIVE → increment 1e adds the local-receipt path: `emit-turn-receipt.sh` → `processed-receipts.jsonl` → `/health.last_processed`. See §3 Amendment 2026-06-26.)*
...
## Amendment (2026-06-27) — routing-doctor calibration learnings from the first live fleet run

The v0.2.39 fleet verification (the first time `fleet`/`routing doctor` ran against the whole live fleet) surfaced a coherent set of calibration refinements. All shipped (or scoped); consolidated here so DR-030 is the accurate single-source.

1. **The `--json` consumer contract (load-bearing — `macf#125`).** `fleet doctor` / `routing doctor` are **health-check CLIs**: **exit code = the health *verdict*** (0 = healthy; non-zero = DEGRADED/unhealthy — but the command *ran fine*), and a successful run (healthy OR degraded) **always emits valid JSON carrying `schema_version`**. Non-JSON / no-`schema_version` = the command itself failed (auth/network/crash). A `--json` consumer (the DR-006 watchdog) MUST **capture output regardless of exit code** and gate probe-ran-vs-failed on **JSON-parse + `schema_version`**, NOT the exit code — else a DEGRADED verdict (the thing to act on) is misread as a probe failure (the bug `macf#125` caught on the live dry-run). `schema_version` (`macf#579`/`#583`) does double duty: hard-version contract AND ran-vs-failed gate. *(Dual-use-signal trap: the exit code carries the verdict, not did-it-run; disambiguate via the result-invariant — valid JSON — per Pattern A.)*

2. **Fleet-scoped vs repo-scoped check tiering (`macf#621`).** Per-agent checks split by data needed: **fleet-scoped** (registry+`/health`-derived: routability, registration-freshness, `instance_id`-match) iterate the **registry** → hold for every registered agent from ANY repo's run; **repo-scoped** (local-config-derived: self-skip `app_name`, session-name) iterate `config.agents` → only where a local `agent-config` entry exists. A **registry-only** agent (e.g. the auditor from another repo) gets fleet-scoped checks + **`null`** on repo-scoped ones (honest-not-asserted), with an **`in_config` provenance flag** distinguishing `null-because-registry-only` from `null-because-check-failed`. Don't gate fleet-scoped checks on the current repo's routing-targets — that silently skips registered agents the repo doesn't route to.

3. **Substrate-shape calibration (`macf#610`/`#614`).** routing-doctor was specced against the **consumer-fleet** shape; the **hand-wired substrate** is a distinct class (v3-channel, **no `agent-config.json`**, install-set ⊋ registered-fleet). Two false-positives, both fixed: **session-name** → **assert-if-present + WARN-not-FAIL** (a missing `agent-config` on a v3 agent is a PASS — `#610`); **pins-consistency** → membership via an explicit **opt-out marker** (`.github/macf-fleet.json {routing_fleet:false}`), default-member, not the raw install-set (`#614`). Discriminator: *consumer shape vs hand-wired-substrate shape* — calibrate for both.

4. **`output_tokens` is null-by-design in `/health.state` (`macf#619`) — candidate for removal.** No CC hook payload exposes per-turn output tokens (they live on the OTLP/`gen_ai.*` pipeline), so `output_tokens` is permanently null in the hook-derived `/health.state`; the watchdog keys on `status`/`elapsed_ms` instead. A future `health_schema_version` bump should **drop the field** rather than carry a never-populated reserved-null.

5. **The telemetry-name convention spans EVERY emission path — lint at the telemetry boundary + centralize the normalization (`macf#587`/#593/#613/#618).** A live Tempo audit (devops, `#587`) found `gen_ai.agent.name` emitted by **two paths**: Claude-native resource attrs (`claude.sh`) AND MACF `invoke_agent`/`SignCsr` spans — the latter leaking the **SCREAMING_SNAKE registry-key** form (`DEVOPS_AGENT`) + the `#538` dual-key drift. Per-site patches (`#593` invoke_agent, `#613` SignCsr) fix the names but recur one-Tempo-trace-at-a-time. Two durable defenses (prevent + detect):
   - **Prevent — centralize the registry-key→kebab normalization at the registry-read boundary:** make `AgentInfo` carry the kebab routing-label as a first-class field, so no telemetry/display consumer handles the raw SCREAMING_SNAKE key (the 3-site leak class: `#593`/`#613`/`#618`). (`fromVariableSegment` round-trips losslessly *because* DR-032 guarantees underscore-free kebab names — the two DRs reinforce.)
   - **Detect — a telemetry-boundary lint:** a Tempo tag-values assert (every `gen_ai.agent.name` matches `^<role>-agent$`, `gen_ai.agent.role` matches `^<role>$`) catches a non-conforming emitter **regardless of which component produced it** — Pattern A at the telemetry boundary, complementing DR-032's static per-repo config-lint. Together: static config-shape + telemetry-boundary Tempo-assert + centralize-at-read. (Folded into DR-032's lint spec too.)
