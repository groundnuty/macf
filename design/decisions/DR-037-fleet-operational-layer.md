...
3. **Most operational surfaces are local-only today** — inventory verified with `macf-devops-agent`, who owns the layer (23 scripts under `fleet/` and `runner/`, **zero `ssh` references**, 16 carrying local-host signals — tmux / systemctl / crontab / `/proc`):

   - `fleet upgrade`, `restart-self`, workspace scans, tmux wake
   - **`fleet/maintenance-lock.sh` — the entry that matters most.** The lock is a file on **local disk**, so a two-host fleet has **two independent locks** and each host believes it holds the only one. The others degrade to *"you must also run it over there"*; this one degrades to **an invariant that reads as held while being violated** — *upgrade ≠ outage* silently stops holding, with nothing surfacing the breach. That is the silent-fallback shape, not a coverage gap.
   - `fleet/runner-watchdog.sh` — runner liveness via local `systemctl`; watches **units, not workspaces**, so it is not covered by "workspace scans"
   - `fleet/install-cron.sh` + `enable-runner-watchdog.sh` — install into the **local crontab**, so every scheduled behaviour in DR-006/DR-031 is per-host and a remote fleet needs the schedule installed on each host, never centrally
   - the entire **`runner/` subsystem** (`install-runner.sh`, `uninstall-runner.sh`, `reconcile-runner.sh`, `verify-runner.sh`, `setup-macf-runner-user.sh`) — local `sudo` provisioning with **no remote form at all**; a separate subsystem from the agent operational layer, not a detail of it

   `#1014` (`apply` naming the deploy steps that remain) is this same shape one phase earlier: the tool completing its own scope while the operator's remaining work goes unstated.

4. **"Local-only" is NOT uniform, and the exception is the strongest evidence for this amendment's framing.** `groundnuty/runner-platform` — an HTTP contract over the tailnet — is **already host-agnostic**: any host that can reach the tailnet provisions, destroys and queries runners, with no `kubectl`, no credential on the caller, and no shell access anywhere. It is the move this amendment predicts for `FleetDriver`, already made once: **a second implementation behind an existing contract, with the remote case falling out rather than being designed in.** A reader taking "the operational layer is local-only" as uniform would be wrong about the newest piece — and wrong about the cheapest available proof that the seam works.
