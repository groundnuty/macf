...
| 3 | `metadata.name` | **RECONCILED** | reported | Derives the App handle (`fleet-manifest.ts:590` `deriveAppHandle`), the control-repo name (`:631`), the router-carrying repo set (`:648`) and `MACF_TRUSTED_ACTORS` (`:616`). The derived handle is compared against **live** GitHub installations: `apply-agent.ts:856` / `:1343` build `ExpectedIdentity{appSlug}`, and `identity-confirm.ts:225` filters live installs by `i.appSlug === expected.appSlug`. Control repo presence observed at `observer.ts:1291`, converged by `apply-fleet.ts` (`provisionControlRepo`). Status renders the derived handle + observed presence per agent (`status.ts:173`, `:232`). |
| 4 | `versions.macf` | **RECONCILED** (upward only) | reported — **but the value shown is `fleet.lock`'s record, not a live host read** (`status.ts:181` ← `observer.ts:1250`) | See §3. Apply's roll observes the agent's live `/health` version (`macf-core/src/fleet-upgrade.ts:571,582-587`); `plan`'s picture of the same field is lock-derived (`observer.ts:1250`). |
...
**(a) `apply` does observe live state.** `apply-version.ts:183-220`
`runApplyVersionPhase` calls `upgradeFleets` (macf-core) with the
manifest-declared target. `macf-core/src/fleet-upgrade.ts:571` reads
...
(`status.ts:181`) and presents it in a column headed `VERSION`, with no
indication that it is a record rather than an observation.

**(c) Convergence is one-directional.**
`macf-core/src/fleet-upgrade.ts:582` —
...
`offline` (`:581`) and is skipped, not rolled — and an offline agent's
...
(`apply-version.ts:33-39`, `unreachable: true`) — which is the normal case
...
2. **`owner.registry` → RECONCILED.** Only the `repo` variant produces a
   live comparison (installation coverage). A stricter reading would say the
   registry is an address, not a value, and therefore cannot be reconciled at
   all. The verdict reflects that `apply` observes a live fact derived from
   the field and drives the fleet toward satisfying it; the scope limit is
   stated in the row.
