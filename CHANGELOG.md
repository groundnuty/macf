...
canonical SessionStart work-pickup hook. Marketplace v0.2.53 re-syncs the plugin
tree (#765 touches `plugin/scripts/check-channel-alive.sh`).

### Reliability
- **#765** — `check-channel-alive.sh` now RETRIES the `/health` probe before the
  loud "CHANNEL-SERVER DEAD" verdict, fixing a single-transient-probe false
  positive that had an agent believing routing was dark while its server was
  fine. Retry count `MACF_CHANNEL_ALIVE_RETRIES` (default 3, floored ≥1 so a
  typo can't skip into a zero-probe false-DEAD); `MACF_CHANNEL_ALIVE_RETRY_DELAY_SECS`
  (default 1s); exits immediately on the first 2xx (zero happy-path latency);
  warns only after ALL attempts fail. Default `MACF_CHANNEL_ALIVE_CURL_TIMEOUT_SECS`
  5→8s. Layered on top of #760's live-pid port-resolution (that block untouched).

### Features
- **#768** — canonical role-aware SessionStart work-pickup hook, generated by
  `macf init`/`update`/`rules refresh` into `settings.json`. Ships the "surface
  pending work + self-nudge to resume" behavior agents previously hand-rolled
  per-workspace in gitignored `settings.local.json` (four divergent ways, one
  with a buggy single-`Enter` tmux submit that never submitted). Thin: delegates
  queue discovery to the plugin `issues` command (mints its own token → no
  attribution-trap, no hardcoded repo list). Role-aware: auto-resume ON for
  actuator roles, OFF for the auditor (DR-026), gated at runtime via
  `MACF_AGENT_ROLE`. Opt-out `MACF_NO_STARTUP_PICKUP=1`; submit via
  `tmux-send-to-claude.sh` (the sanctioned 2-step-Enter helper).
